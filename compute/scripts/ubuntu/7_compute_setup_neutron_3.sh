...
OVERLAY_INTERFACE_IP_ADDRES=$(get_node_ip_in_network "$(hostname)" "mgmt")


iniset_sudo $conf vxlan enable_vxlan true
iniset_sudo $conf vxlan local_ip $OVERLAY_INTERFACE_IP_ADDRESS
#iniset_sudo $conf vxlan local_ip 192.168.100.12
...
if ! sudo sysctl net.bridge.bridge-nf-call-iptables; then
    ...
    echo "# bridge support module added by The dingyz" >> /etc/modules
